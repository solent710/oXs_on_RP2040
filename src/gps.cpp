...
const uint8_t initGpsM6Part1[] = {
    ...
    0xD0, 0x08, 0x00, 0x00, 0x00, 0x96, //        CFG-PRT : Set port to output only UBX (so deactivate NMEA msg) and set baud = 38400.
...
const uint8_t initGpsM6Part2[] = {
    ...
    0x96, //        CFG-PRT : Set port to output only UBX (so deactivate NMEA msg) and set baud = 38400.
    0x00, 0x00, 0x07, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, //        and set baud = 38400.
    0x91, 0x84,                                                 // Check sum                rest of CFG_PRT command
    ...
    // 0xB5,0x62,0x06,0x00,0x14,0x00,0x01,0x00,0x00,0x00,0xD0,0x08,0x00,0x00,0x00,0x96, //        CFG-PRT : Set port to output only UBX (so deactivate
    // NMEA msg) and set baud = 38400.
...
void uboxChecksum()
{ // this function is used to calculate ublox checksum; It must be activated in a line of code below
    uint8_t buffer[] = {
        0xB5, 0x62, 0x06, 0x00, 0x14, 0x00, 0x01, 0x00, 0x00, 0x00,
        0xD0, 0x08, 0x00, 0x00, 0x00, 0x96, //        CFG-PRT : Set port to output only UBX (so deactivate NMEA msg) and set baud = 38400.
...
void GPS::setupGps(void)
{
    if (config.pinGpsTx == 255)
        return; // skip if pin is not defined
    if (config.gpsType == 'U')
    { // send gps config only for U blox with oXs setup (not for cadis nor when when ublox gps is configured externally
...
uint sendGpsConfig(const uint8_t buf[], uint len, uint fromPos)
{
    ...
    while (idx < len)
    {
        if (pio_sm_is_tx_fifo_empty(gpsPio, gpsSmTx))
        {
            ...
            idx++;
            if (buf[idx] == 0XB5)
            { // make a pause when there is a new command (0XB5 = begin )
...
void GPS::handleGpsUblox()
{
    ...
    switch (gpsState)
    {
        ...
        case GPS_WAIT_END_OF_RESET:
            ...
            {
                if (lastActionUs == 0)
                { // we make the init only once
                    uart_tx_program_init(gpsPio, gpsSmTx, gpsOffsetTx, config.pinGpsRx, 9600);
                    lastActionUs = microsRp();
                }
                if ((microsRp() - lastActionUs) > 5000000)
                { // wait at least  5 sec
        ...
        case GPS_M10_IN_RECONFIGURATION:
            if ((microsRp() - lastActionUs) > 5000)
            { // wait at least  4mse between baudrate change
        ...
        case GPS_M6_IN_RECONFIGURATION:
            if (lastActionUs == 0)
            { // last action = 0 means that baudrate has to be rconfigure
                // uart_tx_program_init(gpsPio, gpsSmTx, gpsOffsetTx, config.pinGpsRx, 9600);
                initGpsIdx   = 0; // reset on the first char of the first command to be sent
                lastActionUs = microsRp();
            }
            if ((microsRp() - lastActionUs) > 40000)
            { // wait 40 ms between 2 commands
                if (initGpsIdx >= sizeof(initGpsM6Part2))
                {              // when all bytes have been sent
                    baudIdx++; // use next baudrate
                    if (baudIdx >= 1)
                    { // if text has been sent with all baudrate, we can continue
                ...
                { // when not end of all commands
                    while (initGpsIdx < sizeof(initGpsM6Part2))
                    {
                        if (pio_sm_is_tx_fifo_empty(gpsPio, gpsSmTx))
                        {
                            ...
                            //    Serial.println( pgm_read_byte_near(initGps1 + initGpsIdx ), HEX) ;
                            initGpsIdx++;
                            if (initGpsM6Part2[initGpsIdx] == 0XB5)
                            { // make a pause when there is a new command (0XB5 = begin )
                                // initGpsIdx++;
                                lastActionUs = microsRp();
                                break; // quit the while loop
        ...
        if (queue_try_remove(&gpsRxQueue, &data))
        {
            ...
            switch (_step)
            {
                ...
                case 8:
                    ...
                    GPS_packetCount++;
                    //            printer->print("pac : ");  printer->print(GPS_packetCount); printer->print(",err: "); printer->print(gpsDataErrors);
                    //            printer->print(",skip: "); printer->println(_skip_packet) ;
                    ...
                    if (_class == 0x01)
                        parseGpsUblox(); // we handle only msg from class 01
                                         //    parsed = true;
                                         //}
            }                            // end of case
        }                                // end if
...
bool GPS::parseGpsUblox(void) // move the data from buffer to the different fields
{
    ...
    switch (_msg_id)
    {
        case MSG_POSLLH:
            ...
            new_position = true;
            break;
            //    case MSG_STATUS:                              // !!!!!!!!! I do not see real need of this message because same (and more) data are
            //    in SOL, so this message is not activated in init
            //        next_fix = (_buffer.status.fix_status & NAV_STATUS_FIX_VALID) && (_buffer.status.fix_type == FIX_3D); // si valid position and
            //        speed or 3D fix if (!next_fix)
            //             GPS_fix = false;
            //        break;
        case MSG_SOL: // !!!! here we could also use vertical speed which is I4 in cm/sec)
            ...
            if (_buffer.solution.fix_type == FIX_3D)
                _buffer.solution.satellites += 100; // we add 100 when we have a 3d fix (for Ublox)
        ...
        case MSG_PVT: // this message does not exist in ublox6 (but SOL does not exist in ublox10)
            ...
            // fields[NUMSAT].available = true;
            if (_buffer.pvt.fix_type == FIX_3D)
                _buffer.pvt.satellites += 100; // we add 100 when we have a 3d fix (for Ublox)
            sent2Core0(NUMSAT, _buffer.pvt.satellites);
            if (_buffer.pvt.fix_status & NAV_STATUS_FIX_VALID)
            { // PDOP is valid only when bit 0 =1
                GPS_pdop = _buffer.pvt.position_DOP;
                sent2Core0(GPS_PDOP, _buffer.pvt.position_DOP);
            }
            // if (_buffer.timeutc.flag & 0b111) {
            if ((_buffer.pvt.valid & 0x03) == 0X03)
            { // if date and time are valid (bit 0 and 1 = HIGH)
                // printf("nbr sat : %X \n", GPS_numSat) ;
        ...
        case MSG_VELNED:
            if (GPS_fix)
                sent2Core0(GROUNDSPEED, _buffer.velned.speed_3d);
            ...
            if (GPS_fix)
                sent2Core0(HEADING, _buffer.velned.heading_2d / 1000);
