...
// note : when sensor relies, it sent first 2 bytes with the value = ??????????????? and then it starts a set of bytes depending on the type of
// sensor.
...
void hottPioRxHandlerIrq(); // when a byte is received on the tlm bus, read the pio hott fifo and push the data to a queue (to be processed in the
                            // main loop)
