...
void setupFbus()
{
    ...
    dma_channel_configure(fbus_dma_chan, &fbusDmaConfig,
    ...
    sport_uart_tx_program_init(fbusPio, fbusSmTx, fbusOffsetTx, config.pinPrimIn, 460800,
                               true); // we use the same pin and baud rate for tx and rx, true means thet UART is inverted
...
{ // when a byte is received on the fbus, read the pio fbus fifo and push the data to a queue (to be processed in the main loop)
    // clear the irq flag
    irq_clear(PIO0_IRQ_0);
    uint32_t nowMicros = microsRp();
    while (!pio_sm_is_rx_fifo_empty(fbusPio, fbusSmRx))
    {                                                     // when some data have been received
        ...
        if ((nowMicros - fbusRxMicros) > FBUS_RX_FREE_MICROS)
            c |= 0X8000;
...
{ // main loop : restore receiving mode , wait for tlm request, prepare frame, start pio and dma to transmit it
    ...
    if (restoreFbusPioToReceiveMillis)
    { // put sm back in receive mode after some delay
...
bool processNextInputByte(uint8_t c)
{
    ...
    {
        if ((fbusIsDownlink == false) && (c != FRAME_TYPE_CHANNEL))
        { // for a Rc channel, byte must be FF,
            // not channel data
    ...
    if (fbusLen > 2 && fbusRxBufferIdx == fbusLen)
    { // when all bytes have been received
        ...
        if (chanCount != 16)
        { // at this stage we manage only 16 channels
            fbusRxBufferIdx = 0;
        ...
        if (fbusIsDownlink)
        {
            if ((fbusRxBuffer[1] == SPORT_DEVICEID) && // reply only the physical id = oXs device ID
                (fbusRxBuffer[2] == 0X10))
            { // and type = 0X10 = polling for DATA
...
{ // this code is similar to Sbus in
    ...
    if (((sbusPriMissingFlag == false) && (sbusPriFailsafeFlag == false)) || // copy when frame is OK
        ...
        ((millisRp() - lastSecChannelsMillis) > 50))
    {                                                                       //   or SEC do not exist
...
{   // search for the next data to be sent
