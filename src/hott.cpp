...
void setupHott()
{
    ...
    dma_channel_configure(hott_dma_chan, &hottDmaConfig,
    ...
    hott_uart_tx_program_init(hottPio, hottSmTx, hottOffsetTx, config.pinTlm, 19200,
                              false); // we use the same pin and baud rate for tx and rx, false means thet UART is not inverted
...
void hottPioRxHandlerIrq()
{ // when a byte is received on the tlm pin, read the pio hott fifo and push the data to a queue (to be processed in the main loop)
    // clear the irq flag
    irq_clear(PIO0_IRQ_0);
    while (!pio_sm_is_rx_fifo_empty(hottPio, hottSmRx))
...
#endif
void handleHottRxTx(void)
{ // main loop : restore receiving mode , wait for tlm request, prepare frame, start pio and dma to transmit it
    static uint8_t previous = 0;
    static uint8_t data;
    if (config.pinTlm == 255)
        return; // skip when Tlm is not foreseen
    switch (hottState)
    {
        case RECEIVING:
#ifdef DEBUG_HOTT_WITHOUT_RX // simulate a RX sending a GAM polling every 200 msec
            if ((millisRp() - lastHottRequest) > 200)
            { // to debug simulate a request once per 200msec
            ...
            if (!queue_is_empty(&hottRxQueue))
            {
                ...
                // printf("%X ", data);
                if ((previous == HOTT_BINARY_MODE_REQUEST_ID) && ((data == HOTT_TELEMETRY_GAM_SENSOR_ID) || (data == HOTT_TELEMETRY_GPS_SENSOR_ID)))
                {
                    hottState        = WAIT_FOR_SENDING;
                    hottStartWaiting = microsRp();
                    // printf("r\n");
        ...
        case WAIT_FOR_SENDING:
            ...
            {
                if (sendHottFrame(data))
                { // data must be GAM or GPS; return true when a frame is really being sent
                    hottState        = SENDING;
                    hottStartWaiting = microsRp();
                    // printf("s\n");
                }
...
{ // return true when a frame is being sent
    ...
    if (sendingRequired)
    {                                                                        // when buffer is filled, it can be sent
...
bool fillHottGamFrame()
{
...
#ifdef DEBUG_HOTT_WITHOUT_RX
        if (TxHottData.gamMsg.Battery1 == 0)
            TxHottData.gamMsg.Battery1 = 0XFF;
#endif
    }

    if (fields[RESERVE1].available)
    { // = volt3
        TxHottData.gamMsg.Battery2 = int_round(fields[RESERVE1].value, 100);
    } // battery 2 voltage  0.1V steps. 55 = 5.5V only pos. voltages
    ...
    {
        TxHottData.gamMsg.rpm = int_round(fields[RPM].value, 10);
    ...
    {
        TxHottData.gamMsg.altitude = 500;
    } // altitude in meters. offset of 500, 500 = 0m
    ...
    {
        TxHottData.gamMsg.climbrate_L = 30000;
    } // climb rate in 0.01m/s. Value of 30000 = 0.00 m/s
    TxHottData.gamMsg.climbrate3s = 120; // #28 climb rate in m/3sec. Value of 120 = 0m/3sec

    if (fields[CURRENT].available)
    {
        TxHottData.gamMsg.current = int_round(fields[CURRENT].value, 100);
    } // current in 0.1A steps 100 == 10,0A
...
/*
    if (fields[RESERVE3].available)
    {                                                                            
        TxHottData.gamMsg.fuel_procent = int_round(fields[RESERVE3].value, 10); 
    }
*/    
    ...
    for (uint8_t i = 0; i < (TXHOTTDATA_BUFFERSIZE - 1); i++)
    { // one byte less because the last byte is the checksum
...
{
    if (!gps.gpsInstalled)
        return false; // exit if not installed
    memset(&TxHottData.txBuffer[0], 0, TXHOTTDATA_BUFFERSIZE);
    ...
    if ((gps.GPS_fix_type == 3) || (gps.GPS_fix_type == 4))
    {
        ...
        TxHottData.gpsMsg.flightDirection =
            fields[HEADING].value /
            200; // convert from degre * 100 to 1/2 degree; Flightdir./dir. 1 = 2°; 0° (North), 90° (East), 180° (South), 270° (West)
        uint16_t speedHott;
        ...
        convertLonLat_Hott(fields[LATITUDE].value, &degMin,
                           &decimalMin); // convert to 2 fields (one beging deg*100+min, the other being the decimal part of min with 4 decimals
        ...
        convertLonLat_Hott(fields[LONGITUDE].value, &degMin,
                           &decimalMin); // convert to 2 fields (one beging deg*100+min, the other being the decimal part of min with 4 decimals
        ...
        TxHottData.gpsMsg.HomeDirection =
            gps.GPS_bearing / 2; // Byte 29: HomeDirection (direction from starting point to Model position) (1 byte) 2degree = 1
...
{ // Determine whether a warning has to be sent to the transmitter.
  ...
  // W_IDLE is the IDLE state but that name cannot be used because IDLE is already defined in oXs_out_hott.h.
    ...
    if (state == DEADTIME && millisRp() - warning_start_time > 3000)
        state = W_IDLE;
...
#endif
    if ( flagUpdateHottBuffer ) {        // this flag is set to true when UART get a polling of the device. Then measurement must be filled in the
buffer #ifdef DEBUG_BLINK_UPLOAD_HOTT_DATA blinkLed(5) ; // blink every 500 ms at least #endif
        // fill the buffer with 0
        for ( uint8_t i = 0 ; i < TXHOTTDATA_BUFFERSIZE ; i++ ) {       // first fill the buffer with 0
           TxHottData.txBuffer[i] = 0 ;
        }
        if ( flagUpdateHottBuffer == HOTT_TELEMETRY_GAM_SENSOR_ID  ) {        // this flag is set to true when UART get a polling of the GAM device.
Then measurement must be filled in the buffer TxHottData.gamMsg.start_byte    = 0x7c ; TxHottData.gamMsg.gam_sensor_id = 0x8d ; //GENRAL AIR MODULE =
HOTT_TELEMETRY_GAM_SENSOR_ID TxHottData.gamMsg.sensor_id     = 0xd0 ; TxHottData.gamMsg.stop_byte     = 0x7D ;
...
// in general air module data to fill are:
             ...
             TxHottData.gamMsg.cell[3] =  voltageData->mVoltCell[3] /20 ; // Volt Cell 4 (in 2 mV increments, 210 == 4.20 V)
              ...
              TxHottData.gamMsg.cell[5] =  voltageData->mVoltCell[5] /20 ; // Volt Cell 6 (in 2 mV increments, 210 == 4.20 V)
#endif
#if defined(BATTERY_1_SOURCE) && ( (BATTERY_1_SOURCE == VOLT_1) || (BATTERY_1_SOURCE == VOLT_2) || (BATTERY_1_SOURCE == VOLT_3) || (BATTERY_1_SOURCE
== VOLT_4) || (BATTERY_1_SOURCE == VOLT_5) || (BATTERY_1_SOURCE == VOLT_6) ) && defined(ARDUINO_MEASURES_VOLTAGES) && (ARDUINO_MEASURES_VOLTAGES ==
YES) TxHottData.gamMsg.Battery1 = voltageData->mVolt[BATTERY_1_SOURCE - VOLT_1].value / 100;    //battery 1 voltage  0.1V steps. 55 = 5.5V only pos.
voltages #endif #if defined(BATTERY_1_SOURCE) && ( (BATTERY_1_SOURCE == ADS_VOLT_1) || (BATTERY_1_SOURCE == ADS_VOLT_2) || (BATTERY_1_SOURCE ==
ADS_VOLT_3) || (BATTERY_1_SOURCE == ADS_VOLT_4) ) && defined(AN_ADS1115_IS_CONNECTED) && (AN_ADS1115_IS_CONNECTED == YES ) && defined(ADS_MEASURE)
              TxHottData.gamMsg.Battery1 = ads_Conv[BATTERY_1_SOURCE - ADS_VOLT_1].value / 100;    //battery 1 voltage  0.1V steps. 55 = 5.5V only
pos. voltages #endif #if defined(BATTERY_2_SOURCE) && ( (BATTERY_2_SOURCE == VOLT_1) || (BATTERY_2_SOURCE == VOLT_2) || (BATTERY_2_SOURCE == VOLT_3)
|| (BATTERY_2_SOURCE == VOLT_4) || (BATTERY_2_SOURCE == VOLT_5) || (BATTERY_2_SOURCE == VOLT_6) ) && defined(ARDUINO_MEASURES_VOLTAGES) &&
(ARDUINO_MEASURES_VOLTAGES == YES) TxHottData.gamMsg.Battery2 = voltageData->mVolt[BATTERY_2_SOURCE - VOLT_1].value / 100;    //battery 1 voltage 0.1V
steps. 55 = 5.5V only pos. voltages #endif #if defined(BATTERY_2_SOURCE) && ( (BATTERY_2_SOURCE == ADS_VOLT_1) || (BATTERY_2_SOURCE == ADS_VOLT_2) ||
(BATTERY_2_SOURCE == ADS_VOLT_3) || (BATTERY_2_SOURCE == ADS_VOLT_4) ) && defined(AN_ADS1115_IS_CONNECTED) && (AN_ADS1115_IS_CONNECTED == YES ) &&
defined(ADS_MEASURE) TxHottData.gamMsg.Battery1 = ads_Conv[BATTERY_2_SOURCE - ADS_VOLT_1].value / 100;    //battery 1 voltage  0.1V steps. 55 = 5.5V
only pos. voltages #endif
...
#elif defined(TEMPERATURE_1_SOURCE) && ( (TEMPERATURE_1_SOURCE == VOLT_1 ) || (TEMPERATURE_1_SOURCE == VOLT_2 ) || (TEMPERATURE_1_SOURCE == VOLT_3 )
|| (TEMPERATURE_1_SOURCE == VOLT_4 ) || (TEMPERATURE_1_SOURCE == VOLT_5 ) || (TEMPERATURE_1_SOURCE == VOLT_6 ) )  &&
defined(ARDUINO_MEASURES_VOLTAGES) && (ARDUINO_MEASURES_VOLTAGES == YES) TxHottData.gamMsg.temperature1 = (voltageData->mVolt[TEMPERATURE_1_SOURCE -
VOLT_1].value ) / 10 + 20 ; // Hott applies an offset of 20. A value of 20 = 0°C #else TxHottData.gamMsg.temperature1 = 20 ; // Hott applies an offset
of 20. A value of 20 = 0°C #endif
...
#elif defined(TEMPERATURE_2_SOURCE) && ( (TEMPERATURE_2_SOURCE == VOLT_1 ) || (TEMPERATURE_2_SOURCE == VOLT_2 ) || (TEMPERATURE_2_SOURCE == VOLT_3 )
|| (TEMPERATURE_2_SOURCE == VOLT_4 ) || (TEMPERATURE_2_SOURCE == VOLT_5 ) || (TEMPERATURE_2_SOURCE == VOLT_6 ) ) && defined(ARDUINO_MEASURES_VOLTAGES)
&& (ARDUINO_MEASURES_VOLTAGES == YES) TxHottData.gamMsg.temperature2 = (voltageData->mVolt[TEMPERATURE_2_SOURCE - VOLT_1].value ) /10  + 20 ; // Hott
applies an offset of 20. A value of 20 = 0°C #else TxHottData.gamMsg.temperature2 = 20 ; // Hott applies an offset of 20. A value of 20 = 0°C #endif
...
#endif
#if defined(MAIN_BATTERY_SOURCE) && ( (MAIN_BATTERY_SOURCE == VOLT_1) || (MAIN_BATTERY_SOURCE == VOLT_2) || (MAIN_BATTERY_SOURCE == VOLT_3) ||
(MAIN_BATTERY_SOURCE == VOLT_4) || (MAIN_BATTERY_SOURCE == VOLT_5) || (MAIN_BATTERY_SOURCE == VOLT_6) ) && defined(ARDUINO_MEASURES_VOLTAGES) &&
(ARDUINO_MEASURES_VOLTAGES == YES) TxHottData.gamMsg.main_voltage = voltageData->mVolt[MAIN_BATTERY_SOURCE - VOLT_1].value / 100;          //Main
power voltage using 0.1V steps 100 == 10,0V] / 100 #endif #if defined(ARDUINO_MEASURES_A_CURRENT) && (ARDUINO_MEASURES_A_CURRENT == YES)
...
//                    if( GPS_latAvailable ) {             // test if data are available (GPS fix) ; if available, fill the buffer
//                        GPS_latAvailable = false ;       // reset the flag
                        TxHottData.gpsMsg.flightDirection = GPS_ground_course / 200000 ; // convert from degre * 100000 to 1/2 degree; Flightdir./dir.
1 = 2°; 0° (North), 90° (East), 180° (South), 270° (West) static uint16_t speedHott ; #ifdef GPS_SPEED_3D speedHott = ((uint32_t) GPS_speed_3d) * 36
/1000 ;       // convert from cm/sec to km/h #else speedHott = ((uint32_t) GPS_speed_2d) * 36 /1000 ;       // convert from cm/sec to km/h #endif
                        TxHottData.gpsMsg.GPSSpeedLow = speedHott ;
                        TxHottData.gpsMsg.GPSSpeedHigh = speedHott >> 8 ;
                        uint16_t degMin ;
                        uint16_t decimalMin ;
                        TxHottData.gpsMsg.LatitudeNS = (GPS_lat < 0) ;                        // Byte 10: 000 = N = 48°39’0988
                        convertLonLat_Hott(GPS_lat, & degMin , & decimalMin ) ;              // convert to 2 fields (one beging deg*100+min, the other
being the decimal part of min with 4 decimals TxHottData.gpsMsg.LatitudeMinLow = degMin ;                           // Byte 11: 231 = 0xE7 <= 0x12E7 =
4839 TxHottData.gpsMsg.LatitudeMinHigh = degMin >> 8 ;                     // Byte 12: 018 = 0x12 <= 0x12E7 = 4839 TxHottData.gpsMsg.LatitudeSecLow =
decimalMin ;                           // Byte 13: 220 = 0xDC <= 0x03DC = 0988 TxHottData.gpsMsg.LatitudeSecHigh = decimalMin >> 8 ; // Byte 14: 003 =
0x03 <= 0x03DC = 0988

                        TxHottData.gpsMsg.longitudeEW = (GPS_lon < 0) ;                        // Byte 15: 000  = E= 9° 25’9360
                        convertLonLat_Hott(GPS_lon, &degMin , &decimalMin ) ;              // convert to 2 fields (one beging deg*100+min, the other
being the decimal part of min with 4 decimals TxHottData.gpsMsg.longitudeMinLow = degMin ;                           // Byte 16: 157 = 0x9D <= 0x039D
= 0925 TxHottData.gpsMsg.longitudeMinHigh = degMin >> 8 ;                     // Byte 17: 003 = 0x03 <= 0x039D = 0925
                        TxHottData.gpsMsg.longitudeSecLow = decimalMin ;                           // Byte 18: 144 = 0x90 <= 0x2490 = 9360
                        TxHottData.gpsMsg.longitudeSecHigh = decimalMin >> 8 ;                     // Byte 19: 036 = 0x24 <= 0x2490 = 9360
                        ...
                        TxHottData.gpsMsg.HomeDirection = GPS_bearing / 2 ;                        //Byte 29: HomeDirection (direction from starting
point to Model position) (1 byte) 2degree = 1 altitudeHott += (GPS_altitude / 1000)  ;                                 // convert from mm to m (keep
the ofsset of 500 m)
                   }
 */
...
void convertLonLat_Hott(int32_t GPS_LatLon, uint16_t *degMin, uint16_t *decimalMin)
{
    ...
    GPS_LatLonAbs = (GPS_LatLon < 0 ? -GPS_LatLon : GPS_LatLon) /
                    100; // remove 2 decimals from original value which contains degre with 7 decimals (so next calculation are smaller)
    degre0decimals  = GPS_LatLonAbs / 100000; // extract the degre without  decimal
    minute4decimals = (GPS_LatLonAbs - (((uint32_t)degre0decimals) * 100000l)) *
                      6; // keep the decimal of degree and convert them in minutes (*60) and remove 1 decimal (/10) in order to keep 4 decimals
