...
void handleUSBCmd(void)
{
    ...
    while (1)
    {
        ...
        if (cmdBufferPos >= (CMD_BUFFER_LENTGH - 1))
        { // discard previous char when buffer is full
            cmdBufferPos = 0;
        }
        if (c != '\n')
        {
...
void processCmd()
{
    ...
    // printf("buffer0= %X\n", cmdBuffer[0]);
    if (cmdBuffer[0] == 0x0D)
    { // when no cmd is entered we print the current config
        printConfigAndSequencers();
        return;
    }
    if (cmdBuffer[0] == '?')
    {                      // when help is requested we print the instruction
        ...
        printf("    Gyro Stick Elevator     GSE = YY               "
               "                 elevator \n");
        printf("    Gyro Stick Rudder       GSR = YY               "
               "                 rudder   \n");

        printf("Gyro Gain on Roll           GGR = YYYY          Gain on roll axis (-127/127)\n");
        printf("             Pitch          GGP = YYYY             "
               "    pitch\n");
        printf("             Yaw            GGY = YYYY             "
               "    Yaw\n");
    ...
    if (cmdBuffer[0] != 0x0)
    {
        ...
        if (equalPos != NULL)
        {                    // there is a '=' so search for value
    ...
    if (strcmp("PRI", pkey) == 0)
    {
        ...
        else if ((config.protocol == 'E' || config.protocol == 'F' || config.protocol == 'L') && (!(ui <= 29 or ui == 255)))
        {
    ...
    if (strcmp("CRSFBAUD", pkey) == 0)
    { // if the key is CRSFBAUD
        ui = strtoul(pvalue, &ptr, 10);
        if (*ptr != 0x0)
        {
    ...
    // change debugTlm
    if (strcmp("DEBUGTLM", pkey) == 0)
    { // if the key is DEBUGTLM
    ...
    // change debugSbusOut
    if (strcmp("DEBUGSBUSOUT", pkey) == 0)
    { // if the key is DEBUGSBUSOUT
        if (strcmp("Y", pvalue) == 0)
        {
            debugSbusOut = 'Y';
        }
    ...
    if (strcmp("FVP", pkey) == 0)
    {
        ...
        if (config.protocol == 'E')
            setupExbusList(true); // rebuild the list of fields being sent in jeti exbus
        if (config.protocol == 'J')
            initListOfJetiFields(true); // rebuild the list of fields being sent in jeti ex
        printFieldValues();
    ...
    { //
        ...
        {
            printf("Error : protocol must be S(Sport Frsky), F(Fbus Frsky), C(CRSF=ELRS), J(Jeti), E(jeti Exbus), H(Hott), M(Mpx), 2(Sbus2 Futaba), "
                   "L(SRXL2 Spektrum) or I(Ibus/Flysky)\n");
        }
    }

    // change scale
    if ((strcmp("SCALE1", pkey) == 0) || (strcmp("SCALE2", pkey) == 0) || (strcmp("SCALE3", pkey) == 0) || (strcmp("SCALE4", pkey) == 0))
    {
        ...
        else
        {
            ...
            if (*(pkey + 5) == '1')
            {
                config.scaleVolt1 = db;
            }
            else if (*(pkey + 5) == '2')
            {
                config.scaleVolt2 = db;
            }
            else if (*(pkey + 5) == '3')
            {
                config.scaleVolt3 = db;
            }
            else if (*(pkey + 5) == '4')
            {
                config.scaleVolt4 = db;
            }
            else
            {
    ...
    // change offset
    if ((strcmp("OFFSET1", pkey) == 0) || (strcmp("OFFSET2", pkey) == 0) || (strcmp("OFFSET3", pkey) == 0) || (strcmp("OFFSET4", pkey) == 0))
    {
        db = strtod(pvalue, &ptr);
        if (*ptr != 0x0)
        {
    ...
    */
    ...
    if (strcmp("SETFAILSAFE", pkey) == 0)
    { // if the key is Failsafe
        if (lastRcChannels)
        {
    ...
    if (strcmp("LOGBAUD", pkey) == 0)
    { // if the key is LOGBAUD
        ui = strtoul(pvalue, &ptr, 10);
        if (*ptr != 0x0)
        {
    ...
    {
        if (getPid(0))
        { // true when valid syntax is decoded and pid structure has been updated ;
    ...
    if (strcmp("SEQ", pkey) == 0)
    {
        ...
        {
            if (getAllSequencers())
            {   // true when valid syntax is decoded and seq structure has been updated ;
                // we will save the structure and reboot; during reboot we will check if config is valid
                updateConfig = true;
            }
            else
    ...
    */
    ...
    if (strcmp("A", pkey) == 0)
        printAttitudeFrame(); // print Attitude frame with vario data
    if (strcmp("G", pkey) == 0)
        printGpsFrame(); // print GPS frame
    if (strcmp("B", pkey) == 0)
        printBatteryFrame(); // print battery frame
...
{ // set configIsValid
    ...
    for (uint8_t i = 0; i < 30; i++)
        pinCount[i] = 0; // reset the counter
    configIsValid = true;
    ...
    {
        addPinToCount(config.pinChannels[i]);
    }
    for (uint8_t i = 0; i < 16; i++)
    {
        if (config.pinChannels[i] != 255)
            atLeastOnePwmPin = true;
    }
    for (uint8_t i = 0; i < 4; i++)
    {
        addPinToCount(config.pinVolt[i]);
    }
    ...
    if ((config.pinSda != 255 and config.pinScl == 255) or (config.pinScl != 255 and config.pinSda == 255))
    {
        printf("Error in parameters: SDA and SCL must both be defined or unused\n");
        configIsValid = false;
    }
    if ((config.pinGpsTx != 255 and config.pinGpsRx == 255) or (config.pinGpsRx != 255 and config.pinGpsTx == 255))
    {
        printf("Error in parameters: GPS_TX and GPS_RX must both be defined or unused\n");
        configIsValid = false;
    }
    if ((config.pinSbusOut != 255 and config.pinPrimIn == 255 and config.pinSecIn == 255))
    { // check that when Sbus out is defined, PrimIn is defined too
        printf("Error in parameters: a pin is defined for Sbus Out but not for Primary nor Secondary channels input (PRI or SEC)\n");
        configIsValid = false;
    }
    if ((atLeastOnePwmPin and config.pinPrimIn == 255 and config.pinSecIn == 255))
    { // check that when pwm is defined, PrimIn is defined too
    ...
    {
        printf(
            "Error in parameters: when gyro mode/gain Rc channel is defined (not 255), Rc channels must also be defined for Roll, Pitch and Yaw).\n");
        configIsValid = false;
    }
    ...
    if ((config.gyroChanControl != 255) and (config.gyroChan[0] == config.gyroChanControl or config.gyroChan[1] == config.gyroChanControl or
                                             config.gyroChan[2] == config.gyroChanControl or config.gyroChan[0] == config.gyroChan[1] or
                                             config.gyroChan[0] == config.gyroChan[2] or config.gyroChan[1] == config.gyroChan[2]))
    {
...
{ // print all and perform checks
    ...
    printf("PWM Channels 1, 2, 3 ,4   = %4u %4u %4u %4u (C1 / C16= 0, 1, 2, ..., 15)\n", config.pinChannels[0], config.pinChannels[1],
           config.pinChannels[2], config.pinChannels[3]);
    printf("PWM Channels 5, 6, 7 ,8   = %4u %4u %4u %4u\n", config.pinChannels[4], config.pinChannels[5], config.pinChannels[6],
           config.pinChannels[7]);
    printf("PWM Channels 9,10,11,12   = %4u %4u %4u %4u\n", config.pinChannels[8], config.pinChannels[9], config.pinChannels[10],
           config.pinChannels[11]);
    printf("PWM Channels 13,14,15,16  = %4u %4u %4u %4u\n", config.pinChannels[12], config.pinChannels[13], config.pinChannels[14],
           config.pinChannels[15]);
    printf("Voltage 1, 2, 3, 4        = %4u %4u %4u %4u (V1 / V4 = 26, 27, 28, 29)\n", config.pinVolt[0], config.pinVolt[1], config.pinVolt[2],
           config.pinVolt[3]);
    ...
    else
    {
        ...
        printf("     Chan 1...4  = %5d %5d %5d %5d\n", (int)fmap(config.failsafeChannels.ch0), (int)fmap(config.failsafeChannels.ch1),
               (int)fmap(config.failsafeChannels.ch2), (int)fmap(config.failsafeChannels.ch3));
        printf("     Chan 5...8  = %5d %5d %5d %5d\n", (int)fmap(config.failsafeChannels.ch4), (int)fmap(config.failsafeChannels.ch5),
               (int)fmap(config.failsafeChannels.ch6), (int)fmap(config.failsafeChannels.ch7));
        printf("     Chan 9...12 = %5d %5d %5d %5d\n", (int)fmap(config.failsafeChannels.ch8), (int)fmap(config.failsafeChannels.ch9),
               (int)fmap(config.failsafeChannels.ch10), (int)fmap(config.failsafeChannels.ch11));
        printf("     Chan 13...16= %5d %5d %5d %5d\n", (int)fmap(config.failsafeChannels.ch12), (int)fmap(config.failsafeChannels.ch13),
               (int)fmap(config.failsafeChannels.ch14), (int)fmap(config.failsafeChannels.ch15));
    }
...
void cpyChannelsAndSaveConfig()
{                              // used when pressing boot button to save failsafe value
    config.failsafeType = 'C'; // remove 'H' for HOLD
    memcpy(&config.failsafeChannels, &sbusFrame.rcChannelsData, sizeof(config.failsafeChannels));
    saveConfig();
}
...
void setupConfig()
{ // The config is uploaded at power on
    if (*flash_target_contents == CONFIG_VERSION)
    {
        memcpy(&config, flash_target_contents, sizeof(config));
        if (config.pwmHz == 0XFFFF)
            config.pwmHz = _pwmHz; // set default value when it has not been defined manually
    }
    else
    {
        ...
        config.pwmHz                 = _pwmHz;
        config.gyroChanControl = _gyroChanControl; // Rc channel used to say if gyro is implemented or not and to select the mode and the general
                                                   // gain. Value must be in range 1/16 or 255 (no gyro)
...
void checkSequencers()
{
    ...
    { // process all steps
        ...
        if ((seq.steps[stepIdx].nextSequencerBegin == 1) && (stepIdx > 0))
        { // When the next (not the first one) sequencer begin , close the current
            if (rangeNumber < 2)
            {
        ...
        if (seq.steps[stepIdx].nextSequencerBegin == 1)
        { // for each begin of sequencer
            ...
            if (seqDatasToUpload)
            {
                ...
                seqDatas[seqIdx].currentChValueUs =
                    0; // use a dummy channel value in order to force a change when a channel value will be received from Rx
                seqDatas[seqIdx].lastreceivedRange =
                    dummy; // use a dummy channel value in order to force a change when a channel value will be received from Rx
        ...
        if (seq.steps[stepIdx].chRange > prevRange)
            rangeNumber++; // Count the number of Rc channel values (range) for this sequencer
        ...
        if (seq.defs[seqIdx].type == 1)
        { // when seq has type ANALOG, PWM value must be between 0/100
            if (seq.steps[stepIdx].value < 0 || (seq.steps[stepIdx].value > 100 && seq.steps[stepIdx].value != 127))
            {
                printf("Error in sequencer steps: for sequencer %i  step %i, type is ANALOG(=1); PWM output value must then be in range 0/100 or "
                       "127(=stop)\n",
                       seqIdx + 1, stepIdx + 1);
                configIsValid = false;
                return;
    ...
    if (seq.steps[stepIdx].nextSequenceBegin == 1)
        sequenceIdx++; // count the number of sequence
...
// #define DEBUG_PRINT_SEQDATAS
    ...
    for (uint8_t i = 0; i < seq.defsMax; i++)
    {
        printf("Start=%i End=%i chVal=%i state=%i range=%i first=%i currStep=%i smmoth=%i nextMs=%i outVal=%i\n", seqDatas[i].stepStartAtIdx,
               seqDatas[i].stepEndAtIdx, (int)seqDatas[i].state, seqDatas[i].currentChValue, (int)seqDatas[i].currentRange, seqDatas[i].firstStepIdx,
               seqDatas[i].currentStepIdx, seqDatas[i].smoothUpToMs, seqDatas[i].nextActionAtMs, seqDatas[i].lastOutputVal);
    }
    printf("\n");
#endif
...
void printSequencers()
{
    ...
    for (uint16_t i = 0; i < seq.stepsMax; i++)
    {
        ...
        if (seq.steps[i].nextSequenceBegin == 1)
        {
            ...
            if (seq.steps[i].toRepeat == 1)
            {
                printf(" R");
            }
            if (seq.steps[i].neverInterrupted == 1)
            {
                printf(" U");
            }
            if (seq.steps[i].priorityInterruptOnly == 1)
            {
                printf(" O");
            }
            if (seq.steps[i].isPriority == 1)
            {
                printf(" P");
            }
            printf(" )\n    ");
...
bool getAllSequencers()
{                    // try to get sequencer definition from a string pointed by pvalue; return true if valid
    ...
    { // while not end of value buffer
        ...
        { // this will also parse the sequences and the steps
...
bool parseOneSequencer()
{                                    // try to read a string  with n integer space separated set between [ ]
    ...
    { // try to convert 7 integers
        errno           = 0;
        tempIntTable[i] = strtol(pvalue, &ptr, 10); // convert to integer starting from pvalue; ptr point to the first non converted char; skip
                                                    // whitespace before *ptr = 0 when no error
    ...
    {
        // for each sequence
        if (parseOneSequence() == false)
        { // when true, stepsTemp will be filled for this sequencer
            return false;
        }
...
bool parseOneSequence()
{ // parse one sequence and all steps from this sequence (up to the end or up to next sequence or sequencer)
    // seqIdx, sequenceIdx, stepIdx and stepsTemp[] are used and updated
    char *ptr;
    // look for one sequence and n steps
    if ((*pvalue) != '(')
    { // "(" is used to mark a new sequence
    ...
    pvalue = skipWhiteSpace(pvalue); // skip next spaces
    if ((*pvalue) != '{')
    { // a step befinning with { must exist after a sequence
        printf("Error: after sequence definition %i we must have a steps beginning with {\n", sequenceIdx + 1);
        return false;
    }
    // parse one or several steps
    while (((*pvalue) != 0X00) && ((*pvalue) != '(') && ((*pvalue) != '['))
    { // get one or several steps
        if (parseOneStep() == false)
        { // data are stored in stepsTemp[]
            return false;
        }
...
void printGyro()
{
    ...
    printf("   Channels for :                     mode/gain=%i  ,  Ail stick=%i  ,  Elv stick=%i  ,  Rud stick=%i\n", config.gyroChanControl,
           config.gyroChan[0], config.gyroChan[1], config.gyroChan[2]);
    printf("   Gain per axis (-128/127):                           Roll=%i          Pitch=%i         Yaw=%i\n", config.vr_gain[0], config.vr_gain[1],
           config.vr_gain[2]);
    ...
    printf("   Normal  PIDN= %-5i   %-5i   %-5i       %-5i   %-5i   %-5i      %-5i   %-5i   %-5i\n", config.pid_param_rate.kp[0],
           config.pid_param_rate.ki[0], config.pid_param_rate.kd[0], config.pid_param_rate.kp[1], config.pid_param_rate.ki[1],
           config.pid_param_rate.kd[1], config.pid_param_rate.kp[2], config.pid_param_rate.ki[2], config.pid_param_rate.kd[2]);
    printf("   Hold    PIDH= %-5i   %-5i   %-5i       %-5i   %-5i   %-5i      %-5i   %-5i   %-5i\n", config.pid_param_hold.kp[0],
           config.pid_param_hold.ki[0], config.pid_param_hold.kd[0], config.pid_param_hold.kp[1], config.pid_param_hold.ki[1],
           config.pid_param_hold.kd[1], config.pid_param_hold.kp[2], config.pid_param_hold.ki[2], config.pid_param_hold.kd[2]);
    printf("   Stab.   PIDS= %-5i   %-5i   %-5i       %-5i   %-5i   %-5i      %-5i   %-5i   %-5i\n", config.pid_param_stab.kp[0],
           config.pid_param_stab.ki[0], config.pid_param_stab.kd[0], config.pid_param_stab.kp[1], config.pid_param_stab.ki[1],
           config.pid_param_stab.kd[1], config.pid_param_stab.kp[2], config.pid_param_stab.ki[2], config.pid_param_stab.kd[2]);
    printGyroMixer();
}

void printGyroMixer()
{ // this function is also called at the end of the gyroMixer calibration process
    ...
    else
    {
        ...
        for (uint8_t i = 0; i < 16; i++)
        {
            ...
            {
                printf(
                    "Channel %-2i center=%-4i rollRight=%-4i rollLeft=%-4i pitchUp%-4i pitchDown=%-4i yawRight=%-4i yawLeft=%-4i min=%-4i max=%-4i\n",
                    i + 1, pc(gyroMixer.neutralUs[i] - 1500), pc(gyroMixer.rateRollRightUs[i]), pc(gyroMixer.rateRollLeftUs[i]),
                    pc(gyroMixer.ratePitchUpUs[i]), pc(gyroMixer.ratePitchDownUs[i]), pc(gyroMixer.rateYawRightUs[i]), pc(gyroMixer.rateYawLeftUs[i]),
...
{ // The config is uploaded at power on
    ...
    else
    {
        ...
        for (uint8_t i = 0; i < 16; i++)
        { // set all mixer as unsued (for gyro) and with default values
...
bool getPid(uint8_t mode)
{ // get all pid parameters for one mode; return true if valid; config is then updated
    // we expect getting 9 parameters (uint16) kp ki kd for ail, then for elv and for rud; all are space delimited
    char *ptr;                       // get the pos of first non converted integer
    pvalue = skipWhiteSpace(pvalue); // skip space at the begining
    int32_t tempTable[9];
    for (uint8_t i = 0; i < 9; i++)
    { // try to convert 9 integers
        errno        = 0;
        tempTable[i] = strtol(pvalue, &ptr, 10); // convert to integer starting from pvalue; ptr point to the first non converted char; skip
                                                 // whitespace before *ptr = 0 when no error
        if ((ptr == pvalue) || (ptr == NULL))
        {
    ...
    pvalue = skipWhiteSpace(pvalue); // skip space at the begining
    if ((*pvalue) != 0)
    { // last char must be 0 (end of string)
