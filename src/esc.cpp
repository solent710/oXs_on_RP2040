...
void setupEsc()
{
    if (config.pinEsc == 255)
        return; // skip when there is no ESC
...
{   // when a byte is received on the esc bus, read the pio esc fifo and push the data to a queue (to be processed in the main loop)
    // clear the irq flag
    irq_clear(PIO0_IRQ_1);
    uint32_t nowMicros = microsRp();
    while (!pio_sm_is_rx_fifo_empty(escPioRx, escSmRx))
    {                                                                    // when some data have been received
        ...
        if ((nowMicros - lastEscReceivedUs) > escFreeTimeUs)
            c |= 0X8000; // add a flag when there is a gap between char
        if (queue_try_add(&escRxQueue, &c) == false)
            printf("Esc queue is full\n"); // push to the queue
...
void handleEsc()
{
    ...
    if (config.pinEsc == 255)
        return; // skip when esc is not foreseen
    ...
    while (!queue_is_empty(&escRxQueue))
    {
        ...
        if (escRxBufferIdx == escMaxFrameLen)
...
#endif
    if (config.escType == HW4)
    { // when frame is received for Hobbywing V4
        processHW4Frame();
    }
    else if (config.escType == KONTRONIK)
    {
...
void processHW4Frame()
{
    if (escRxBuffer[0] == 0x9B)
    {
        ...
        if (throttle < 1024 && pwm < 1024 && rpm < 200000 && escRxBuffer[11] < 0xF && escRxBuffer[13] < 0xF && escRxBuffer[15] < 0xF &&
            escRxBuffer[17] < 0xF)
        {
        ...
        //        escRxBuffer[13] & 0xF0 || escRxBuffer[15] & 0xF0 || escRxBuffer[17] & 0xF0 || escRxBuffer[1] == 0x9B)
            //        // escRxBuffer[1] == 0x9B added by mstrens to perhaps avoid info frame; in principe LEN is different and so should already be
            //        omitted
            //{
            //}
            // else
            ...
            { // when volt1 is defined, we discard voltage from esc
            ...
            if (config.pinVolt[1] == 255)
            {
                ...
                if (currentf < 0)
                    currentf = 0;
                if (currentf < ESC_MAX_CURRENT)
                { // discard when current is to high
            ...
            if ((config.pinVolt[2] == 255) or ((config.temperature != 1) and (config.temperature != 2)))
            { //  we discard temp from esc
                sent2Core0(TEMP1, calcTemp((float)tempFet));
            }
            if ((config.pinVolt[3] == 255) or (config.temperature != 2))
            { //  we discard temp from esc
                sent2Core0(TEMP2, calcTemp((float)tempBec));
            }

            // printf("Esc throttle=%i   pwm=%i   Volt=%i  current=%i  consumed=%i  temp1=%i  temp2=%i\n", throttle , pwm , voltage , (int) current,
            // (int) escConsumedMah , (int) tempFet , (int) tempBec );
